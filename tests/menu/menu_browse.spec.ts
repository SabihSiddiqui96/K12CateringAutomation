...
test.describe('Menu - Browse, Search & Cart', () => {
  ...
  test('Menu - Search by name, clear, and category/allergen filters work', async () => {
    ...
    await expect(catering.locator('h2, h3').filter({ hasText: /\d+ items?/ }).first()).toBeVisible();
    ...
    await catering.waitForTimeout(500);
    await expect(catering.getByRole('heading', { name: new RegExp(realCategory, 'i') })).toBeVisible();
  ...
  test('Menu - Cancel closes modal without adding; confirm adds item to cart and shows Grand Total', async () => {
    ...
    await expect(catering.locator('div.w-80.p-5.rounded-xl.fixed')).toContainText(/Grand Total:/);
  });
