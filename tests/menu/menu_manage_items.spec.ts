...
test.describe('Menu - Add & Edit Items', () => {
  ...
  async function closeModal() {
    // The combined "Cancel and close modal" button was replaced by the "Close modal" (X) button on
  ...
  test('Menu - Add Menu Item button is visible and modal opens with all required fields', async () => {
    await expect(catering.getByRole('button', { name: 'Add a new menu item' })).toBeVisible();
  ...
  test('Menu - Edit modal opens pre-filled; Cancel discards, Save updates item name and price', async () => {
    ...
    await expect(editDialog()).not.toBeVisible();
    await expect(catering.locator('#main-content').getByText('$9.99').first()).toBeVisible();
